...
本项目是一个基于 Streamlit 的小学至高中数学试题图片批改 Demo。用户可以直接在网页中上传带有学生答案的题目照片，系统将图片发送给硅基流动提供的多模态大模型，完成题目识别、答案核对、错因分析和订正建议生成。
...
当前提示词覆盖小学、初中和高中数学。除口算、分数、小数、单位换算、应用题和基础几何外，也支持初高中代数、方程与不等式、函数、三角、数列、平面与立体几何、解析几何、概率统计等题型，以及带有手写答案的作业、练习册或试卷照片。
...
页面中的年级选项覆盖小学一年级至高中三年级，共 12 个选项。选择结果不会改变图片本身，而是通过 `grade` 参数写入用户 Prompt。例如选择“高中二年级（高二）”时，模型会收到：
...
| 解题方法 | 小学阶段避免不必要的方程和高阶公式；初高中阶段使用对应课程中的代数、函数、几何和概率方法 |
| 语言难度 | 小学低年级更口语化、步骤更直观；初高中可使用规范的数学符号、定义和定理 |
| 错因分类 | 结合年级判断是口诀、进位退位、单位、公式、定义域、分类讨论或推导等错误 |
| 订正步骤 | 使用该年级更容易理解和模仿的方法 |
| 学习建议 | 推荐与该年级知识点相匹配的练习方向 |

一般情况下，同一道题的最终正确答案不会因为年级不同而改变；变化主要体现在解释方式和推荐方法。例如小学低年级可能使用连续加法、画图或实物分组，初高中可能使用方程、函数、向量或规范的定理推导。
...
```text
学生是小学四年级，但这是一道五年级提前学习题；请用四年级能理解的方法讲解。
```

年级只是一条提示信息，模型仍可能偶尔使用超纲方法。对教学方法有严格要求时，应在补充说明中明确限制，例如“不要使用方程”“请用画线段图的方法”“只使用高一已经学习的函数知识”。
...
```json
{
  ...
  "messages": [
    {
      "role": "system",
      "content": "你是一位严谨、耐心的数学老师，负责批改小学一年级至高中三年级的数学题……"
    },
    {
      "role": "user",
      "content": [
        ...
        {
          "type": "text",
          "text": "请批改以上图片中的数学题，题目范围可能从小学到高中……"
        }
      ]
    }
  ],
...
测试使用模拟 API 响应，不消耗硅基流动额度。页面还使用 Streamlit `AppTest` 做过渲染检查，模型下拉框包含 15 个选项且无页面异常。
...
- 当前没有用户登录、历史记录和数据库。
- 当前没有自动保存上传图片或批改报告。
- 当前提示词覆盖小学至高中，但竞赛题、大学先修内容和证明要求较高的题目仍可能需要人工复核。
